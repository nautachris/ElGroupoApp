...
EditEventAttendees = {
    ConfirmSave: function () {
        var updatedAttendees = EditEventAttendees.GetAttendeeData();
        if ($("#IsRecurring").val() == "True") {
            ConfirmRecurrence('Do you want to update all recurring events with these changes?', function () {
                EditEventAttendees.Save(updatedAttendees, true);
            }, function () {
                EditEventAttendees.Save(updatedAttendees, false);
            });
        }
        else {
            Confirm('Do you want to save your attendee changes?', function () {
                EditEventAttendees.Save(updatedAttendees, false);
            });
    ...
    Save: function (attendees, recurring) {
        //list of adds
        //list of deletes
        //list of changes 
        ...
        console.log('save attendee changes');
        console.log(model);
        $.ajax({
            url: "/Events/EditEventAttendees",
            type: 'POST',
            ...
            success: function success(results) {
                EditEventAttendees.UnInit();
                $("#divViewAttendees").empty().html(results).show();
    ...
    AddSelectedUser: function () {
        ...
        var id = -1;
        var attendee = null;
        if ($("#txtAttendeeSearch")[0].hasAttribute('data-group-id')) {
            isGroup = true;
            id = Number($("#txtAttendeeSearch").attr('data-group-id'));
            attendee = {
                UserGroupId: id,
                IsOrganizer: false,
                IsRegistered: true
            ...
            isGroup = false;
            id = Number($("#txtAttendeeSearch").attr('data-user-id'));
            attendee = {
                UserId: id,
                IsOrganizer: false,
                IsRegistered: true
        ...
        $.ajax({
            url: "/Events/GetAttendeeListItemHTML",
            type: 'POST',
            contentType: "application/json",
            ...
            success: function success(results) {
                $("#txtAttendeeSearch").val('');
                $("#btnAddSelectedUser").hide();
                $("#divAttendeeList").append(results);
            },
            error: function error(err) {
                alert('error');
            }
    ...
    AddInvitedUser: function () {
        //var attendees = EditEventAttendees.GetAttendeeData();
        console.log('add invited user');
        var attendee = {
    ...
    GetAttendeeData: function () {
        //problem is that the razor function we're using to append data attributes results in an extra ""
        var attendees = [];
        $("#divAttendeeList div.attendee-container").each(function () {
            //this works
            var isOrganizer = $(this).find('img').hasClass('img-event-organizer');
            //this works
            var isInvitedUser = $(this).attr('data-attendee-type') === 'invite';
            var name = null;
            var email = null;
            if (isInvitedUser) {
                //this works
                name = $(this).attr('data-new-user-name');
                email = $(this).attr('data-new-user-email');
            }
            //group id will never be here
            //var groupId = null;
            var eventAttendeeId = null;
            if ($(this).attr('data-attendee-type') === 'attendee') {
                console.log('has data-event-attendee-id');
                eventAttendeeId = Number($(this).attr('data-attendee-id'));
            }
            else {
                console.log('does not have data-event-attendee-id');
            }
            var userId = null;
            if ($(this).attr('data-attendee-type') === 'user') {
                userId = Number($(this).attr('data-attendee-id'));
                console.log('has data-user-id');
            }
            else {
                console.log('does not have data-user-id');
            }
            var userGroupId = null;
            if ($(this).attr('data-attendee-type') === 'group') {
                userGroupId = Number($(this).attr('data-attendee-id'));

            }
            else {
                console.log('does not have data-group-id');
            }
    ...
    UpdateAttendeeList: function (attendees) {

    },
    ToggleOrganizer: function () {
        if ($(this).attr('data-is-editable') !== 'False') {
            if ($(this).hasClass('img-event-organizer')) {
                $(this).removeClass('img-event-organizer');
            }
            else {
                $(this).addClass('img-event-organizer');
            }
        }
    ...
    RemoveAttendee: function () {
        $(this).closest('div.attendee-container').remove();
    },
    UnInit: function () {
        $("#divEditAttendees").off("click", "div.attendee-info img", EditEventAttendees.ToggleOrganizer);
        $("#divEditAttendees").off("click", "img.delete-attendee", EditEventAttendees.RemoveAttendee);

    },
    Init: function () {
        EditEventAttendees.EventId = Number($("#EventId").val());
        //none of these need delegates b/c we're loading 
        ...
        $("#btnInviteNewUser").on("click", function () {
            EditEventAttendees.AddInvitedUser();
        });
        $("#txtAttendeeSearch").on("input", EditEventAttendees.EventHandlers.AttendeeSearchTextChanged);
        $("#btnAddSelectedUser").on("click", EditEventAttendees.AddSelectedUser);
        $("#divEditAttendees").on("click", "div.attendee-info img", EditEventAttendees.ToggleOrganizer);
        $("#divEditAttendees").on("click", "img.delete-attendee", EditEventAttendees.RemoveAttendee);
        $("#txtNewUserEmail").on("change", function () {
            if ($(this).val() !== '' && $("#txtNewUserName").val() !== '') {
                console.log('seeting to enabled');
                $("#btnInviteNewUser").show();
            }
            else {
                $("#btnInviteNewUser").hide();
            }
        });
        $("#txtNewUserName").on("change", function () {
            if ($(this).val() !== '' && $("#txtNewUserEmail").val() !== '') {
                console.log('seeting to enabled');
                $("#btnInviteNewUser").show();
            }
            else {
                console.log('seeting to disabled');
                $("#btnInviteNewUser").hide();
            }
        });
    ...
    InitAutocomplete: function () {
        $("#txtAttendeeSearch").autocomplete({
            minLength: 3,
            autoFocus: true,
            delay: 300,
            select: function (e, i) {
                console.log(i);
                if (i.item.group === true) {
                    $("#txtAttendeeSearch").removeAttr('data-user-id');
                    $("#txtAttendeeSearch").attr('data-group-id', i.item.id);
                }
                else {
                    $("#txtAttendeeSearch").removeAttr('data-group-id');
                    $("#txtAttendeeSearch").attr('data-user-id', i.item.id);
                }

                $("#btnAddSelectedUser").show();
            },
